...
export class ConversationService {
  ...
  handleGetAll(pagination: Pagination) {
    ...
    this.http.get<Array<Conversation>>(`${environment.serverUrl}/conversations`
      , {params})
      .subscribe({
        next: conversations => {
  ...
  handleCreate(conversationToCreate: ConversationToCreate) {
    this.http.post<Conversation>(`${environment.serverUrl}/conversations`, conversationToCreate)
      .subscribe({
        next: conversation => {
  ...
  handleDelete(conversationPublicId: string) {
    ...
    this.http.delete<string>(`${environment.serverUrl}/conversations`, {params})
      .subscribe({
  ...
  handleGetOne(conversationId: string) {
    ...
    this.http.get<Conversation>(`${environment.serverUrl}/conversations/get-one-by-public-id`,
      {params})
      .subscribe({
        next: conversation => {
          this.mapDateToDayJs(conversation);
  ...
  handleMarkAsRead(conversationId: string): void {
    const params = new HttpParams().set("conversationId", conversationId);
    this.http.post<number>(`${environment.serverUrl}/conversations/mark-as-read`, {}, {params})
      .subscribe({
